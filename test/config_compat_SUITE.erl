...
config(_Config)  ->
    ...
    ?ok_legacy_option(set_cfg_value([invalid], [], ?GGSN_CONFIG)),

    ?error_option(set_cfg_value([accept_new], invalid, ?GGSN_CONFIG)),
    Accept0 = (catch ergw_config_legacy:validate_config(?GGSN_CONFIG)),
    ?equal(true, maps:get(accept_new, Accept0)),
    Accept1 = (catch ergw_config_legacy:validate_config(set_cfg_value([accept_new], true, ?GGSN_CONFIG))),
    ?equal(true, maps:get(accept_new, Accept1)),
    Accept2 = (catch ergw_config_legacy:validate_config(set_cfg_value([accept_new], false, ?GGSN_CONFIG))),
    ...
    SockCfg = (catch ergw_config_legacy:validate_config(
    ...
    ?match({error, {apn, _}},
	   (catch ergw_config_legacy:validate_config(
    ...
    APN0 = maps:get(apns, (catch ergw_config_legacy:validate_config(?GGSN_CONFIG))),
