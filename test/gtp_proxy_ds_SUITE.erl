...
map(_Config) ->
    ...
    ProxyInfo0 = #proxy_info{ggsns = [#proxy_ggsn{apn = [<<"apn0">>]}],
                             imsi = <<"000000000000000">>, 
                             msisdn = <<"unknown">>},
    ?match({ok, ProxyInfo0}, gen_server:call(gtp_proxy_ds_test, {map, ProxyInfo0})),

    ProxyInfo1 = #proxy_info{ggsns = [#proxy_ggsn{apn = [<<"apn1">>]}],
                             imsi = <<"111111111111111">>, 
                             msisdn = <<"unknown">>},
    ?match({ok, #proxy_info{ggsns = [#proxy_ggsn{apn = [<<"example1">>, <<"com">>]}],
                            imsi = <<"111111111111111">>, 
                            msisdn = <<"16477141111">>}}, 
           gen_server:call(gtp_proxy_ds_test, {map, ProxyInfo1})),

    ProxyInfo2 = #proxy_info{ggsns = [#proxy_ggsn{apn = [<<"apn2">>]}],
                             imsi = <<"222222222222222">>, 
                             msisdn = <<"unknown">>},
    ?match({ok, #proxy_info{ggsns = [#proxy_ggsn{apn = [<<"example2">>, <<"com">>]}], 
