...
-define(HUT, ergw_oam_handler).				%% Handler Under Test

-define(idleTimeout, 100000).	%% change idle timeout value to 100 secs.
...
oam_get(Config) ->
    ...
    % Check that only HTTP/2 is supported, trying HTTP/1.1
    ?match({400, #{cause := <<"UNSUPPORTED_HTTP_VERSION">>}},
	   json_http_request(Protocol, get, URI, [{"ergwPars", "Idle-Timeout"}])),

    {GtpC1, _, _} = create_session(ipv4, Config),
    % fire a json http2 req via the gun created client
    ?match({200, #{'Idle-Timeout' := _}}, 
	       json_h2_request(H2C, get, URI, [{"ergwPars", "Idle-Timeout"}])),
    ?match({400, _}, json_h2_request(H2C, get, URI, [{"ergwPars", "test"}])),
...
oam_post(Config) ->
    ...
    % ?match({400, #{cause := <<"UNSUPPORTED_HTTP_VERSION">>}},
    % 	      json_http_request(Protocol, post, URI, bodyErr)),

    {GtpC1, _, _} = create_session(ipv4, Config),
    
    % Apns = ergw_oam_handler:get_apn_par('Idle-Timeout'),
    ErgwPars = timeout_map(),
    Body = jsx:encode(ErgwPars),
    ?match({200, #{'Idle-Timeout' := _}},
	   json_h2_request(H2C, post, URI, Body)),
...
% start a HTTP/1.1 client with erlang httpc module
json_http_request(_Protocol, Method, URI, Query) ->
...
% start a HTTP/2 client with gun 
h2_connect(Protocol) ->
    Port = ranch:get_port(ergw_http_listener),
...
% Build json HTTP/2 get request 
json_h2_request(H2C, Method, #{path := Path}, Query) when Method =:= get ->
...
% Build json HTTP/2 post request
json_h2_request(H2C, Method, #{path := URL}, Body0)
	when Method =:= post;
		 Method =:= put ->
	Headers = [{<<"accept">>, <<"application/json, application/problem+json">>}],
	StreamRef = gun:Method(H2C, URL, Headers, Body0),
	case gun:await(H2C, StreamRef) of
	{response, fin, Code, _Headers} ->
	    {Code, empty};
	{response, nofin, Code, _Headers} ->
		{data, fin, Body} = gun:await(H2C, StreamRef),
		ct:pal("~ncode = ~p, Body = ~p~n",[Code, Body]),
		JSON = case Body of
		       <<>> -> empty;
		       _    -> jsx:decode(Body, [return_maps, {labels, attempt_atom}])
		   end,
...
timeout_map() ->
	#{ergwPars => #{'Idle-Timeout' => ?idleTimeout}}.

%Setting different timeouts per APN not considered at the moment
% set_timeouts('Idle-timeout', APNs) ->
%     Keys = maps:keys(APNs),
%     update_timeout(Keys, APNs, ?idleTimeout).

% update_timeout([], Map, _Timeout) ->
%     #{ergwPars => #{'Idle-Timeout' => Map}};
% update_timeout([Key | Rest], Map, Timeout) ->
%     update_timeout(Rest, maps:put(Key, Timeout, Map), Timeout).
