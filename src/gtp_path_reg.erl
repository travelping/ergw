...
-export([register_tei/1, unregister_tei/1]).
-export([all/0]).
...
register_tei(TEI) ->
       regine_server:register(?SERVER, self(), TEI, tei).

unregister_tei(TEI) ->
       regine_server:unregister(?SERVER, TEI, tei).
...
handle_register(Pid, TEI, tei, State) ->
    case ets:insert_new(?SERVER, {{tei, TEI}, Pid}) of
	true ->  {ok, [{tei, TEI}], State};
...
handle_unregister(TEI, tei, State) ->
    unregister({tei, TEI}, State);
