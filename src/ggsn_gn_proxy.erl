...
handle_event(state_timeout, _, #c_state{session = connecting} = State, Data) ->
    delete_forward_session(normal, Data),
    {next_state, State#c_state{session = shutdown, fsm = busy}, Data}.
...
handle_request(ReqKey,
	       #gtp{type = create_pdp_context_request,
		    ie = IEs} = Request, _Resent, #c_state{session = SState} = State,
...
handle_response({Direction, From}, #gtp{type = delete_pdp_context_response},
		_Request, #c_state{session = shutdown_initiated} = State, Data) ->
...
initiate_pdp_context_teardown(sgsn2ggsn = Direction, From, #c_state{session = connected},
			      #{right_tunnel := Tunnel,
...
initiate_pdp_context_teardown(ggsn2sgsn = Direction, From, #c_state{session = SState},
			      #{left_tunnel := Tunnel,
