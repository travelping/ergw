...
-ifdef(TEST).
-export([load/1, validate_config/1]).
-endif.
...
load() ->
    load(setup:get_all_env(ergw)).

load(Term) ->
    Config = validate_config(Term),
    {ok, maps:remove('$setup_vars', Config)}.
...
validate_option(handlers, Value) when is_list(Value), length(Value) >= 1 ->
    ...
    validate_options(fun validate_handlers_option/1, Value, [], map);
validate_option(node_selection, Value) when ?is_opts(Value) ->
    check_unique_keys(node_selection, Value),
    validate_options(fun validate_node_selection_option/1, Value, [], map);
validate_option(nodes, Value) when ?non_empty_opts(Value) ->
    ...
    Opts = validate_options(validate_nodes(_, NodeDefaults), Value1, [], map),
    #{default => Defaults, entries => Opts};
...
validate_option(http_api, undefined) ->
    #{enabled => false};
...
validate_handlers_name(Name, Values)
  when Name =:= gn;
       Name =:= s5s8 ->
    Id = erlang:phash2(Values),
    iolist_to_binary(io_lib:format("~s-~w", [Name, Id]));
validate_handlers_name(Name, _)
  when is_atom(Name); is_binary(Name) ->
    ergw_config:to_binary(Name).

validate_handlers_option({Opt, Values0})
  when ?is_opts(Values0) ->
    Name = validate_handlers_name(Opt, Values0),
    ...
    {Name, Handler:validate_options(Values)};

validate_handlers_option(Opts) ->
    throw({error, {options, Opts}}).

validate_node_selection_option({Key, {Type, Opts}})
  when (is_binary(Key) orelse is_atom(Key)) andalso is_atom(Type) ->
    {ergw_config:to_binary(Key), {Type, ergw_node_selection:validate_options(Type, Opts)}};
validate_node_selection_option(Opt) ->
...
validate_nodes({Name, Opts}, Defaults)
  when (is_binary(Name) orelse is_list(Name)), ?is_opts(Opts) ->
    {ergw_config:to_binary(Name), validate_options(fun validate_node_option/2, Opts, Defaults, map)};
validate_nodes(Opt, _) ->
    throw({error, {options, Opt}}).
