...
path_restart_recovery(Config) ->
    ...
    {GtpC2, _, _} = create_pdp_context(random, gtp_context_inc_restart_counter(GtpC1)),
...
pfcp_sx_association_metric(_Config) ->
    ?equal(ok, prometheus_registry:register_collector(ergw_core, ergw_prometheus_collector)),
    Metrics = prometheus_text_format:format(ergw_core),
...
load_ocs_config(Initial, Update) ->
    ergw_test_lib:load_aaa_answer_config([{{gy, 'CCR-Initial'}, Initial},
