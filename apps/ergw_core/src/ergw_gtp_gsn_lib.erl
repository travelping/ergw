...
create_session_m(APN, PAA, DAF, {Candidates, SxConnectId}, SessionOpts0) ->
    do([statem_m ||
	   ...
	   statem_m:lift(ct:pal("APNOpts: ~p~n", [APNOpts])),
...
authenticate() ->
    do([statem_m ||
	   ...
	   _ = ct:pal("AuthResponse: ~p", [Response]),
	   {SessionOpts, AuthSEvs} <- statem_m:lift(Response),
	   _ = ct:pal("SessionOpts: ~p~nAuthSEvs: ~pn", [SessionOpts, AuthSEvs]),
	   statem_m:modify_data(_#{session_opts => SessionOpts}),
	   return(AuthSEvs)
       ]).
...
reselect_upf(Candidates, APNOpts, UPinfo) ->
    do([statem_m ||
	   _ = ct:pal("~s", [?FUNCTION_NAME]),
	   SessionOpts <- statem_m:get_data(maps:get(session_opts, _)),
	   statem_m:lift(ct:pal("SessionOpts: ~p~n", [SessionOpts])),
...
add_apn_timeout(APNOpts) ->
    do([statem_m ||
	   _ = ct:pal("~s", [?FUNCTION_NAME]),
	   statem_m:lift(ct:pal("AddApnTimeout: ~p~n", [APNOpts])),
...
assign_local_data_teid(Key, PCtx, NodeCaps, RightBearer) ->
    do([statem_m ||
	   _ = ct:pal("~s", [?FUNCTION_NAME]),
	   statem_m:lift(ct:pal("AssignLocalDataTeid")),
...
gx_ccr_i(Now) ->
    do([statem_m ||
	   ...
	   Response <- statem_m:wait(ReqId),
	   _ = ct:pal("Gx CCR-I Response: ~p", [Response]),
...
gy_ccr_i(Now) ->
    do([statem_m ||
	   ...
	   Response <- statem_m:wait(ReqId),
	   _ = ct:pal("Gy CCR-I Response: ~p", [Response]),
...
pfcp_create_session(Now, PCtx0, GyEvs, AuthSEvs, RfSEvs, PCCErrors0) ->
    do([statem_m ||
	   ...
	   _ = ct:pal("Before Wait (~p): ~p", [self(), D2]),

	   Response <- statem_m:wait(ReqId),
	   _ = ct:pal("PFCP: ~p", [Response]),

	   D1 <- statem_m:get_data(),
	   _ = ct:pal("Before Response (~p): ~p", [self(), D1]),
...
pfcp_create_session_response(Response) ->
    do([statem_m ||
	   _ = ct:pal("~s", [?FUNCTION_NAME]),
	   D1 <- statem_m:get_data(),
	   _ = ct:pal("After Response (~p): ~p", [self(), D1]),
	   _ = ct:pal("Response: ~p", [Response]),
