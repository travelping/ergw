...
release_local_teid(RecordId, _,
		   #tunnel{socket = Socket, local = #fq_teid{teid = TEID}} = Tunnel) ->
    gtp_path:unbind_tunnel(Tunnel, RecordId),
    ergw_tei_mngr:release_tei(Socket, TEID);
release_local_teid(_, PCtx, #bearer{local = #fq_teid{teid = TEID}})
  when PCtx /= undefined ->
    ergw_tei_mngr:release_tei(PCtx, TEID);
release_local_teid(RecordId, PCtx, M) when is_map(M) ->
    lists:foreach(release_local_teid(RecordId, PCtx, _), maps:values(M));
release_local_teid(_, _, _) ->
    ok.

release_local_teids(#{record_id := RecordId} = Data) ->
    PCtx = maps:get(pfcp, Data, undefined),
    Keys = [left_tunnel, right_tunnel, bearer],
    lists:foreach(
      fun(K) when is_map_key(K, Data) ->
	      release_local_teid(RecordId, PCtx, maps:get(K, Data));
	 (_) ->
	      ok
      end, Keys),
    ok;
