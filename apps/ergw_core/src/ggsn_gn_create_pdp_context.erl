...
create_pdp_context_fail(ReqKey, Request, Error, State, Data) ->
    _ = ct:pal("~s", [?FUNCTION_NAME]),
...
path_bind(Request) ->
    do([statem_m ||
	   ...
	   LeftTunnel <- statem_m:return(gtp_path:bind(Request, LeftTunnel0)),
...
init_session(IEs) ->
    do([statem_m ||
	   _ = ct:pal("~s", [?FUNCTION_NAME]),
	   #{left_tunnel := LeftTunnel, context := Context, aaa_opts := AAAopts} <- statem_m:get_data(),

	   SessionOpts0 = ggsn_gn:init_session(IEs, LeftTunnel, Context, AAAopts),
	   SessionOpts1 = ggsn_gn:init_session_from_gtp_req(IEs, AAAopts, LeftTunnel, SessionOpts0),
