...
update_pdp_context_ok(ReqKey,
    ...
    ct:pal("IEs: ~p~nTunnel: ~p~nBearer: ~p~nContext: ~p~n",
...
update_pdp_context_fun(#gtp{type = update_pdp_context_request, ie = IEs} = Request, State, Data) ->
    statem_m:run(
      do([statem_m ||
	     ...
	     update_tunnel_endpoint(Request, LeftTunnelOld),
...
update_tunnel_endpoint(Request, LeftTunnelOld) ->
    do([statem_m ||
	   ...
	   LeftTunnel <- statem_m:return(ergw_gtp_gsn_lib:update_tunnel_endpoint(
					   Request, LeftTunnelOld, LeftTunnel0)),
...
collect_charging_events(IEs) ->
    do([statem_m ||
	   _ = ct:pal("~s", [?FUNCTION_NAME]),
	   #{'Session' := Session, left_tunnel := LeftTunnel} <- statem_m:get_data(),
	   {OldSOpts, NewSOpts} = ggsn_gn:update_session_from_gtp_req(IEs, Session, LeftTunnel),
