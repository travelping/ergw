...
-import('smc_test_lib', [lib_init_per_group/1,
			  ...
			  update_app_config/3,
			  load_config/1]).
-import('smc_test_lib', [meck_init/1,
			  meck_init_hut_handle_request/1,
			  meck_reset/1,
			  meck_unload/1,
			  meck_validate/1]).
-import('smc_test_lib', [init_seq_no/2,
...
-import('smc_test_lib', [start_gtpc_server/1, start_gtpc_server/2,
...
-import('smc_test_lib', [set_cfg_value/3, add_cfg_value/3]).
-import('smc_test_lib', [outstanding_requests/0, wait4tunnels/1, wait4contexts/1,
			  active_contexts/0, hexstr2bin/1, maps_key_length/2]).
-import('smc_test_lib', [get_metric/4]).
...
-define(PGW_U01_SX_IPv6, {16#fd96, 16#dcd2, 16#efdb, 16#41c3, 0, 0, 16#20, 1}).
-define(PGW_U02_SX_IPv6, {16#fd96, 16#dcd2, 16#efdb, 16#41c3, 0, 0, 16#20, 2}).
-define(TDF_U_SX_IPv6, {16#fd96, 16#dcd2, 16#efdb, 16#41c3, 0, 0, 16#30, 1}).
...
-define(match_map(Expected, Actual), smc_test_lib:match_map(Expected, Actual, ?FILE, ?LINE)).

-define(match_metric(Type, Name, LabelValues, Expected),
	smc_test_lib:match_metric(Type, Name, LabelValues, Expected, ?FILE, ?LINE, 10)).
